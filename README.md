...
- **同じ出来事について各社が別記事を出した場合、直近30分間で通知できるのは最大2件まで**にして流れすぎを防ぎます（30分以上通知が途絶えると、また最大2件まで通知できる新しい枠が開きます。詳細は「5-5」参照）。
...
これに対して `dedup.py` が「タイトルの見た目の類似度」で記事を話題ごとにグルーピングし、**「直近30分間に通知できるのは最大2件まで」というスライディングウィンドウ方式**で間引きます:

1. 同じ話題について、最初の2件（最速で報じた2社分）はそのまま通知する（タイトルは書き換えない。記事自身のタイトルにすでに「続報」等が付いていればそのまま表示される）。
2. その後30分以内に来た同じ話題の記事は、既読化だけして**通知しない**（ここがノイズ削減の本体）。
3. 通知が30分以上途絶えると「新しい枠」が開き、再びそこから最大2件まで通知できるようになる。事件が「発生→死亡→逮捕」のように展開していく場合、各段階でまた最大2件ずつ拾えるということ。
...
- `dedup_followup_minutes`: 通知が途絶えてから、これだけ経つと新しいウィンドウ(枠)が開く分数。実行時刻(wall clock)基準（省略時は **30分**）。cron-job.orgからの起動が1時間ごとの場合、実質的に「同じ実行内の重複は必ずブロック、次の実行(≒1時間後)の新展開はまた最大2件まで通す」という動きになる。
