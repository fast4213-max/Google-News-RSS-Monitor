...
- **同じ出来事について各社が別記事を出した場合、最速2件だけ通知し、以降は60分以上経った「続報」だけを通知します**（それ以外は既読化のみで間引かれます。詳細は「5-5」参照）。
...
2. 3件目以降は、**その話題を最後に通知した記事の時刻から60分以上経っている場合のみ**「続報」として通知する（タイトル先頭に `続報: ` が付く）。
3. 60分経たないうちに来た同じ話題の記事は、既読化だけして**通知しない**（ここがノイズ削減の本体）。
...
```json
{
  ...
  "webhook_env": "DISCORD_WEBHOOK_URL_DAITO",
  "dedup_first_n": 2,
  "dedup_followup_minutes": 60,
  "dedup_similarity_threshold": 0.28
}
```

- `dedup_first_n`: 同じ話題についてそのまま通知する最速N件（省略時は **2件**）。
- `dedup_followup_minutes`: 「続報」として再通知するまでに必要な経過時間・分（省略時は **60分**）。
