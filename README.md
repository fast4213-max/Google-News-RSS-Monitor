...
> ⚠️ **地名の除外で解決するのは「共通点が検索キーワードだけ」のケースです。** 文の言い回し自体が似ている無関係な記事（例:「大東市長が記者会見で表明 新体育館の建設へ」と「大東市長が記者会見で陳謝 職員の不祥事を受けて」＝地名除外後も 0.36）は、依然として同じ話題と誤判定されます。bigram類似度だけで区別できる限界のため、この種の誤判定が気になる場合は `dedup_ignore_words` に `"市長"` `"記者会見"` のような共通語を足して対処してください（上の例は 0.36 → 0.15 まで下がります）。
