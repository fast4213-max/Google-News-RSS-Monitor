...
- **同じ出来事について各社が別記事を出した場合、最速2件だけ通知し、以降は実行時刻基準で30分以上経った「続報」だけを通知します**（それ以外は既読化のみで間引かれます。詳細は「5-5」参照）。
...
「大東市」のような狭い範囲のフィードは既定値（1回10件まで、3日より古い記事は通知しない）で十分ですが、「中東情勢」のように**話題が広く更新が速いフィード**を追加する場合は、そのフィードだけ設定を変えたいことがあります。`feeds.json` に任意項目として追加できます:
...
- `stale_days`: このフィードだけ、何日より古い記事を「既読化のみ」にするかを変える（省略時は **3日**）。

**この2つは省略可能です。** 書かなければ今まで通り「10件まで／3日フィルタ」が使われるので、既存の `daito` フィードなど他のフィードには一切影響しません。
...
これに対して `dedup.py` が「タイトルの見た目の類似度」で記事を話題ごとにグルーピングし、以下のルールで間引きます:

1. 同じ話題について、**最初の2件**（最速で報じた2社分）はそのまま通知する。
2. 3件目以降は、**このシステムが最後にその話題を通知した「実行時刻(wall clock)」から30分以上経っている場合のみ**、`続報: ` を付けて通知する（発生→死亡→逮捕のような本当の続報を拾うため）。
3. 30分経っていない同じ話題の記事は、既読化だけして**通知しない**（ここがノイズ削減の本体）。
   - ポイントは、経過時間を**記事自身のpubDateではなく、必ずこのプログラムを実際に処理した時刻**で測っていること。Googleニュースは記事のpubDateが不正確なことがあり（古い記事を後から再インデックスする等）、記事のpubDate同士を比較すると「1回の実行でたまたま届いた、pubDateが数時間ズレている古い重複記事」まで続報として誤通知してしまう問題があったため。
   - この方式なら「1回の実行内で処理された記事」は必ずまとめて間引かれ、次回の定期実行（1時間ごと）になって初めて続報として通知されます。
...
```json
{
  ...
  "webhook_env": "DISCORD_WEBHOOK_URL_DAITO",
  "dedup_first_n": 2,
  "dedup_followup_minutes": 30,
  "dedup_similarity_threshold": 0.28
}
```

- `dedup_first_n`: 同じ話題についてそのまま通知する最速N件（省略時は **2件**）。
- `dedup_followup_minutes`: 「続報」として再通知するまでに必要な経過時間・分。実行時刻(wall clock)基準（省略時は **30分**）。cron-job.orgからの起動が1時間ごとの場合、実質的に「同じ実行内の重複は必ずブロック、次の実行(≒1時間後)の新展開は必ず通す」という動きになる。
...
> ⚠️ タイトルの類似度による簡易判定のため完全ではありません。「本当に新しい続報(逮捕・死亡等)」と「単なる他社の後追い報道」を意味的に区別しているわけではないので、30分経った後追い記事が続報として通知されることもあります（安全側に倒して「本来間引きたい記事が漏れて通知される」方向に倒している）。
...
A. Googleニュースの検索結果は、記事の公開日とは無関係に「Google側が最近クロール/再インデックスした記事」が急に上位に出てくることがあります。このシステムは既読管理を記事のID(guid)だけで行っているため、初めて見る記事であればどれだけ古くても本来は拾ってしまいます。**対策済み**: `main.py` で記事の `pubDate` をチェックし、**指定日数（デフォルト3日）より古い記事は「既読化のみ」して通知しない**ようにしています。全体のデフォルト値を変えたい場合は `main.py` 冒頭の `DEFAULT_STALE_ARTICLE_DAYS = 3` を、特定のフィードだけ変えたい場合は `feeds.json` にそのフィードの `stale_days` を書いてください（「5-4」参照）。なお `pubDate` が無い/パースできない記事は誤って握りつぶさないよう、安全側に倒して通知されます。
