...
1. 同じ話題について、**配信元が特定できた記事の最初の3件**（最速で報じた3社分）はそのまま通知する（タイトルは書き換えない。記事自身のタイトルにすでに「続報」等が付いていればそのまま表示される）。このとき各記事の**配信元名**（タイトル末尾の `- 媒体名` 表記）と**pubDate**を記録しておく。
   - この「最速3件」の枠は、**配信元が特定できた記事だけでカウントする**。配信元が特定できない記事（`- 媒体名` 表記が無いタイトル）は、これとは**別の枠**（`dedup_unknown_source_limit`、デフォルト3件）でカウントされ、そちらもそのまま通知する。こうすることで、例えば1件目のタイトルがたまたま配信元不明な形式でも、それに枠を1つ奪われず「配信元が分かる記事」を確実に3件確保できる（続報判定に使える手がかりを最大限残すため）。
2. 4件目以降(配信元が分かる記事について)は、**「過去に通知した配信元と同じ配信元」が「その配信元の前回pubDateより新しい内容」で改めて記事を出した場合にだけ**続報として通知する。壁時計での待機時間は設けていない（pubDateが新しければ即座に通知される）。
   ...
   - 配信元が特定できない記事は、`dedup_unknown_source_limit` の枠を使い切った後は、それ以上通知されない(続報判定のしようが無いため)。
...
```json
{
  ...
  "webhook_env": "DISCORD_WEBHOOK_URL_DAITO",
  "dedup_first_n": 3,
  "dedup_unknown_source_limit": 3,
  "dedup_similarity_threshold": 0.17
}
```

- `dedup_first_n`: 同じ話題について、配信元が特定できた記事をそのまま通知する最速件数（省略時は **3件**）。
- `dedup_unknown_source_limit`: 同じ話題について、配信元が特定できない記事をそのまま通知する件数。`dedup_first_n`とは別枠（省略時は **3件**）。
- `dedup_similarity_threshold`: タイトルを「同じ話題」とみなす類似度のしきい値、0〜1（省略時は **0.17**）。値を上げるほど「同じ話題」の判定が厳しくなり間引かれにくくなる（=ノイズが増える）。下げるほど間引かれやすくなるが、無関係な記事を誤って同じ話題とみなすリスクが上がる。
  - 0.28だった頃、「メモ・ノートが見つかった」のような**言い回しの違う続報**が類似度不足で別の新しい話題と誤判定され、最初から3件通知される事故が実際に起きた（元の類似度は0.18〜0.34程度）。無関係な話題同士の類似度は通常0.1未満に収まるため、0.17まで下げても誤って無関係な話題を同じ話題とみなすリスクは低いまま、言い回しが変わった続報を拾いやすくしている。

> ⚠️ 配信元名は、タイトル末尾の `- 媒体名` 表記から抜き出している。この表記が無い(またはGoogleニュース側で表記が変わる)タイトルは配信元が特定できず、`dedup_unknown_source_limit` の枠を使い切った後は続報判定の対象にならない(=それ以上通知されない)。
>
