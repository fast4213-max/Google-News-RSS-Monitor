...
| `saigai` | 自然災害 | `DISCORD_WEBHOOK_URL_SAIGAI` | `max_per_run: 100`（Googleニュースが返す上限とほぼ同じ＝実質上限なし） / `dedup_first_n: 0`（**1話題あたりの件数上限なし**） |

**自然災害フィードについて:**
- 検索キーワードは `地震 OR 津波 OR 台風 OR 大雨 OR 洪水 OR 土砂災害 OR 噴火 OR 特別警報 OR 避難指示` です。増減させたい場合は `feeds.json` の `url` を編集してください（ブラウザのアドレスバーに `https://news.google.com/rss/search?q=キーワード&hl=ja&gl=JP&ceid=JP:ja` と入力するとURLエンコード済みのURLが得られます）。
