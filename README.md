...
- **同じ出来事について各社が別記事を出した場合、最速2件だけ通知し、それ以降は同じ話題である限り通知しません**（既読化のみで間引かれます。詳細は「5-5」参照）。
...
1. 同じ話題について、**最初の2件**（最速で報じた2社分）だけそのまま通知する。
2. 3件目以降は、**時間が経っても再通知せず**、既読化だけして**通知しない**（ここがノイズ削減の本体）。
   （Googleニュースは同じ出来事の記事を何時間も経ってから急に再インデックスすることがあるため、
   「一定時間経てば続報として再通知する」方式は採用していない。単に同じ話題の重複記事を
   延々と拾ってしまうだけになりやすいため。）
3. 話題ごとの記録（クラスタ）は `state/clusters_<feed_id>.json` に保存され、**72時間**経過すると自動的に破棄される（何日も経ってから無関係な記事が誤って同じ話題に混ざるのを防ぐため。逆に言うと、72時間より後に本当に新しい続報が来た場合は、また最初の2件として通知される）。
...
> ⚠️ タイトルの類似度による簡易判定のため完全ではありません。言い回しが大きく異なる記事は別話題として扱われ、意図せず追加で通知されることがあります（安全側に倒して「本来間引きたい記事が漏れて通知される」方向に倒している）。
