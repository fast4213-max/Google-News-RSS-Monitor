...
「大東市」のような狭い範囲のフィードは既定値（1回10件まで、3日より古い記事は通知しない、3時間より古い記事は後追いなら通知しない）で十分ですが、「中東情勢」のように**話題が広く更新が速いフィード**を追加する場合は、そのフィードだけ設定を変えたいことがあります。`feeds.json` に任意項目として追加できます:
