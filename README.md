...
- **同じ出来事について各社が別記事を出した場合、最速2件だけ通知し、以降は30分以上経った「続報」だけを通知します**（それ以外は既読化のみで間引かれます。詳細は「5-5」参照）。
...
2. 3件目以降は、**その話題を最後に通知した記事の時刻から30分以上経っている場合のみ**「続報」として通知する（タイトル先頭に `【続報】` が付く）。
3. 30分経たないうちに来た同じ話題の記事は、既読化だけして**通知しない**（ここがノイズ削減の本体）。
4. 話題ごとの記録（クラスタ）は `state/clusters_<feed_id>.json` に保存され、**72時間**経過すると自動的に破棄される（何日も経ってから無関係な記事が誤って同じ話題に混ざるのを防ぐため）。
...
- `dedup_followup_minutes`: 「続報」として再通知するまでに必要な経過時間・分（省略時は **30分**）。
...
> ⚠️ タイトルの類似度による簡易判定のため完全ではありません。言い回しが大きく異なる続報は別話題として扱われ通知されることがあります（安全側に倒して「本来間引きたい記事が漏れて通知される」方向に倒している）。
