...
| id | name | 通知先 | 設定 |
|---|---|---|---|
| `daito` | 大東市 | `DISCORD_WEBHOOK_URL_DAITO` | 既定値（1回50件まで / 1話題あたり最速3件+30分クールダウン） + `dedup_ignore_words: ["大阪府"]` |
...
- 検索キーワードは `大東市 AND (大雨警報 OR 警報 OR 避難指示 OR 高齢者等避難 OR 土砂災害 OR 浸水 OR 氾濫 OR 地震 OR 台風 OR 津波)` です。「【レベル３大雨警報】大阪府・大東市に発表」のような、**大東市に直接関わる警報・避難情報を確実に拾うための専用フィード**です。
...
- なお `daito`（大東市の一般ニュース）フィードも同じ記事を拾うため、**大雨警報の記事は大東市チャンネルと自然災害チャンネルの両方に届くことがあります**。気になる場合は言ってください。
