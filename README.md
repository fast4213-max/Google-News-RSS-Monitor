...
- **同じ出来事について各社が別記事を出した場合、最速3件だけ通知**し、それ以降は**同じ配信元が前回より新しい内容で改めて報じた記事(続報)だけ**を通知します（詳細は「5-5」参照）。
...
これに対して `dedup.py` が「タイトルの見た目の類似度」で記事を話題ごとにグルーピングし、**「最速3件」＋「配信元ベースの続報判定」**で間引きます:

1. 同じ話題について、最初の3件（最速で報じた3社分）はそのまま通知する（タイトルは書き換えない。記事自身のタイトルにすでに「続報」等が付いていればそのまま表示される）。このとき各記事の**配信元名**（タイトル末尾の `- 媒体名` 表記）と**pubDate**を記録しておく。
2. 4件目以降は、**「過去に通知した配信元と同じ配信元」が「その配信元の前回pubDateより新しい内容」で改めて記事を出した場合にだけ**続報として通知する。壁時計での待機時間は設けていない（pubDateが新しければ即座に通知される）。
   - 考え方: 同じ配信元が同じ話題について中身の変わらない記事を再送してくることは基本的に無いため、「同じ配信元 かつ pubDateが前回より新しい」であれば、ほぼ確実に内容が更新された(=本当の続報)とみなせる。
   - 逆に、**最速3件に含まれていない初見の配信元**が4件目以降に出てきても通知しない。「その記事が本当に新しい内容を報じているのか、単に他社の後追い(内容は同じ)なのか」を区別する手段が無いため、安全側に倒している。
3. **公開から `fresh_hours` 時間以上経った記事**は、すでに通知済みの話題であれば、上記の最速枠・続報判定を問わず一律で通知しない（Googleニュースが古い記事を後から再インデックスしてくることがあるための対策）。まだ一度も通知していない話題であれば、古くても通知する(見逃し防止)。
...
- `dedup_first_n`: 同じ話題についてそのまま通知する最速件数（省略時は **3件**）。
...
> ⚠️ 配信元名は、タイトル末尾の `- 媒体名` 表記から抜き出している。この表記が無い(またはGoogleニュース側で表記が変わる)タイトルは配信元が特定できず、続報判定の対象にならない(=最速3件の後は通知されない)。
>
