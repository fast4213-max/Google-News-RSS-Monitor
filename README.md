...
- `dedup_similarity_threshold`: タイトルを「同じ話題」とみなす類似度のしきい値、0〜1（省略時は **0.12**）。値を上げるほど「同じ話題」の判定が厳しくなり間引かれにくくなる（=ノイズが増える）。下げるほど間引かれやすくなるが、無関係な記事を誤って同じ話題とみなすリスクが上がる。
  - 無関係な話題同士の類似度は通常0.1未満に収まるため、0.12まで下げても誤って無関係な話題を同じ話題とみなすリスクは低いまま、言い回しが変わった続報や、既存記事がpubDateだけ更新されて再配信されたケースを拾いやすくしている。
